...
async function sendMessages(messages: OpenAI.ChatCompletionMessageParam[]) {
    const fineTunedMessages: OpenAI.ChatCompletionMessageParam[] =[
        { role: 'system', content: '너는 영화 정보를 알려주는 챗봇이야 어떤 방식으로든 답변에서의 제목은 항상 {{한글제목// 소문자영어제목}}(출시년도)처럼 {{}}기호로 제목을 묶어 // 기호로 한글과 영어 제목을 구분하고 ()기호로 영화의 출시년도를 표시해줘 그리고 한글 제목이 정확하지 않다면 억지로 지어내지 말고 {{abc xyz}}(1234)처럼 영화 제목만 표시해줘'},
        { role:'assistant', content:'영화 {{어벤져스//avengers(2019)}}는 국제적으로 큰 주목을 받은 안소니 루소 감독의 작품이에요. 인피니티 워 이후 절반만 살아남은 지구 마지막 희망이 된 어벤져스 먼저 떠난 그들을 위해 모든 것을 걸은 스토리를 가진 액션 SF 작품이죠'},
